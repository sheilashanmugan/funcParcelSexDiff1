...
tiff("/Users/sheilash/Desktop/projects/pfn_sex_diff/paper/figures/barplots/Gam_sig_vert_barplot.tiff", width = 4.8, height = 3.5, units = 'in', res = 300)
ggplot(sums_all_col_rank, aes(x = network, y = vertecies, fill=network, alpha=factor(sex)))+
  ...
  scale_alpha_manual(values = c("male"=0.5, "female"=1), guide='none') + scale_fill_manual(values = colormap$netColorF) + scale_y_continuous(expand = c(0, 0), limits=c(0,1600)) +
