...
#get subject list
Behavior_data<- as.data.frame(readMat('/Volumes/projects/funcParcelSexDiff/inputData/Behavior_693.mat'))
bblidList <- as.list(Behavior_data$BBLID)

#directories with single subject atlas hard labels
mask1dir <- "/Volumes/projects/funcParcelSexDiff/data/Revision/SingleParcellation/SingleAtlas_Analysis/Atlas_Visualize/Group_AtlasLabel_Network_"
mask2dir <- "/Volumes/projects/funcParcelSexDiff/data/Revision/SingleParcellation/SingleAtlas_Analysis/Atlas_Visualize"
...
for(j in 1:17){
  print(j);
  dicepath <- paste0("/Volumes/projects/funcParcelSexDiff/Revision/inputData/diceResults/dice_results_", j, ".csv")
...
for(i in 1:17){
  print(i);
  dicepath <- paste0("/Volumes/projects/funcParcelSexDiff/Revision/inputData/diceResults/dice_results_", i, ".csv")
...
tiff("violin_plot_dice.tiff", width = 8, height = 2.5, units = 'in', res = 300)
