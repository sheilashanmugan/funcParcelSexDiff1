...
ResultFolder <- '/data/jux/BBL/projects/exec_extern_sex/pfn_sex_diff/results';
...
for (i in 1:17)
{
  ...
  for (j in 1:FeaturesQuantity_NonZero)
  {
    print(j)
    Loading_Data <- as.numeric(Data_I_NonZero[,j]);
    Gam_Analysis <- gam(Loading_Data ~  oSex + Motion + s(AgeYears, k=4)+ s(AgeYears, by = oSex, k=4) , method = "REML", data = Behavior_New);
    Gam_P_Vector[j] <- summary(Gam_Analysis)$s.pv[2];
    Gam_Z_Vector[j] <- qnorm(Gam_P_Vector[j] / 2, lower.tail = FALSE);
    lm_Analysis <- lm(Loading_Data ~ oSex + Motion + AgeYears + AgeYears*oSex, data = Behavior_New);
    Int_T <- summary(lm_Analysis)$coefficients[5, 3];
    if (Int_T < 0) {
      Gam_Z_Vector[j] <- -Gam_Z_Vector[j];
    }
  }
  ...
  Gam_Z_Bonf_Sig_Vector_All <- matrix(0, 1, FeaturesQuantity);
  Gam_Z_Bonf_Sig_Vector_All[NonZero_ColumnIndex] <- Gam_Z_Bonf_Sig_Vector;
  ResultantFolder = paste0(ResultFolder, '/GamAnalysis/AtlasLoading/SexByAgeEffects');
  dir.create(ResultantFolder, recursive = TRUE);
  writeMat(paste0(ResultantFolder, '/SexByAgeEffect_AtlasLoading_17_Network_', as.character(i), '.mat'), Gam_P_Vector_All = Gam_P_Vector_All, Gam_Z_Vector_All = Gam_Z_Vector_All, Gam_P_FDR_Vector_All = Gam_P_FDR_Vector_All, Gam_Z_FDR_Sig_Vector_All = Gam_Z_FDR_Sig_Vector_All, Gam_P_Bonf_Vector_All = Gam_P_Bonf_Vector_All, Gam_Z_Bonf_Sig_Vector_All = Gam_Z_Bonf_Sig_Vector_All);
}
