...
Xgenes <- subset(chromGeneRefined1, chromRefined == "X", select= "geneRefined")
Ygenes <- subset(chromGeneRefined1, chromRefined == "Y", select= "geneRefined")

geneMatChrom_X_200 <- geneMatChrom_200[colnames(geneMatChrom_200) %in% Xgenes$geneRefined]
geneMatChrom_X_200_t <- as.data.frame(t(geneMatChrom_X_200))

geneMatChrom_Y_200 <- geneMatChrom_200[colnames(geneMatChrom_200) %in% Ygenes$geneRefined]
geneMatChrom_Y_200_t <- as.data.frame(t(geneMatChrom_Y_200))

t.test_results <- mapply(t.test, x= geneMatChrom_X_200_t, y = geneMatChrom_Y_200_t, SIMPLIFY = F)



XY_ttest <- data.frame(matrix(NA,nrow=200,ncol=2))
colnames(XY_ttest)<- c("parcel", "tstat")

for (i in 1:200){
  XY_ttest[i, 1] <- colnames(geneMatChrom_X_200_t[i])
  tmp <- t.test(geneMatChrom_X_200_t[i], geneMatChrom_Y_200_t[i])
  XY_ttest[i,2] <- tmp$statistic
}

cor.test(data_parc_lh_sort_200$wt_mean, XY_ttest$tstat)
  
XMeans <- colMeans(geneMatChrom_X_200_t)
cor.test(data_parc_lh_sort_200$wt_mean, XMeans)

YMeans <- colMeans(geneMatChrom_Y_200_t)
cor.test(data_parc_lh_sort_200$wt_mean, YMeans)
