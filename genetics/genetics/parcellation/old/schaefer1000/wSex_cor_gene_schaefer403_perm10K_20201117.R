...
# read in LH parc
lh_schaefer1000dt <- read.table("/cbica/projects/funcParcelSexDiff/inputData/genetics/sensitivity_analyses/parcellation/data/genes/parcellations/github/fsaverage5/lh_Schaefer1000_17net_fs5_L.txt")
lh_schaefer1000 <- lh_schaefer1000dt[1:10242,]
...
for (i in 1:500){
  for (j in 1:500){
    if (data_parc_lh_parcnum$lh_schaefer1000[i] == lh_schaefer1000dt$V1[j]){
...
pval1 <- read.csv( "/cbica/projects/funcParcelSexDiff/results/genetics/permutations/img100_schaefer1000_perm10K_SortParcNum_gene_permSphereP_df_20201102.csv")
pval <- pval1[,-1]
...
sigY <- subset(dfsig, chromRefined == "Y")
...
Xgenes <- subset(sigX, chromRefined == "X", select= "gene")
Ygenes <- subset(sigY, chromRefined == "Y", select= "gene")
...
colnames(XY_ttest)<- c("parcel", "tstat")
...
cor.test(data_parc_lh_sort_500_comp$wt_mean, XY_ttest$tstat)

XMeans <- colMeans(geneMatChrom_X_200_t)
cor.test(data_parc_lh_sort_500_comp$wt_mean, XMeans)

YMeans <- colMeans(geneMatChrom_Y_200_t)
cor.test(data_parc_lh_sort_500_comp$wt_mean, YMeans)
...
XY_ttest_vert <-data.frame(matrix(NA,nrow=1,ncol=10242))
rownames(XY_ttest_vert)<- "XY_ttest_vert"
...
for(g in 1:length(namelist)){
  ...
  for (i in 1:500){
    for (j in 1:10242){
      if (gene_var_mat$V1[i] == lh_schaefer1000[j]){
