...
set.seed(1)
#geneMatChrom_200_sub <- as.data.frame(geneMatChrom_200[,1:10])
img_gene_permSphereP <- mclapply(geneMatChrom_200, FUN=perm.sphere.p, y=data_parc_lh_sort_200_vec, perm.id= schaefer_perm1k, corr.type='pearson', mc.set.seed =  TRUE, mc.cores=getOption("mc.cores", 20))



write.csv(img_gene_permSphereP, "/cbica/projects/funcParcelSexDiff/results/genetics/permutations/img100_gene_permSphereP_df_20200721.csv")
