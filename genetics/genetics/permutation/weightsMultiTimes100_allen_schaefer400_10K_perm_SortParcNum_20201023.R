...
setDT(lh_schaefer400_ct_200, keep.rownames = "parcNum")
...
data_parc_lh <- subset(data_parc_lh, lh_schaefer400 != "65793")
data_parc_lh_parcnum <- data_parc_lh
data_parc_lh_parcnum$parcNum <- "NA"

for (i in 1:200){
  for (j in 1:200){
    if (data_parc_lh_parcnum$lh_schaefer400[i] == lh_schaefer400_ct_200$V1[j]){
      data_parc_lh_parcnum$parcNum[i] <- lh_schaefer400_ct_200$parcNum[j] }}}
...
data_parc_lh_sort_200_comp_vec <- as.vector(data_parc_lh_parcnum_sort$wt_mean)

set.seed(1)
img_gene_permSphereP <- mclapply(geneMatChrom_200, FUN=perm.sphere.p, y=data_parc_lh_sort_200_comp_vec, perm.id= schaefer_perm1k, corr.type='pearson', mc.set.seed =  TRUE, mc.cores=getOption("mc.cores", 2))


write.csv(img_gene_permSphereP, "/cbica/projects/funcParcelSexDiff/results/genetics/permutations/img100_schaefer400_perm10K_SortParcNum_gene_permSphereP_df_20201023.csv")
