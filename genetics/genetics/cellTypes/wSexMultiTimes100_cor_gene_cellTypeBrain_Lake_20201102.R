...
###read in gene data
geneC <- read.csv("/cbica/projects/funcParcelSexDiff/inputData/genetics/genes_20647_schaefer_V2.csv",header=FALSE)
genebrain <- read.csv("/cbica/projects/funcParcelSexDiff/inputData/genetics/cellTypes/brain_genes_HPA.csv", header = FALSE)
geneA <- genebrain$V1[genebrain$V1 %in% geneC$V1]

gene<- as.data.frame(geneA)
colnames(gene) <- "V1"
...
#chech cell type enrichement 
cell <- read.csv("~/cbica/projects/funcParcelSexDiff/inputData/genetics/cellTypes/Lake18_celltypes.csv")
cell2 <- subset(cell, grepl("^Ex", cell$Cluster) | grepl("^In", cell$Cluster) )
...
#Ranked these 68 genes/270 observations based on correlation 
...
#subtract the overall median rank of the 270 observations from the median rank for each celltype
sigt4<- sigt3 - median(dfsig$corrSigRanked) # for the average correlation ranking for each cell type, how far away it is from the median
sigt4
sigt5a <- as.data.frame(sigt4)
setDT(sigt5a, keep.rownames = "cellType")
sigt5a$cellType <- as.factor(sigt5a$cellType)

sigt5 <- subset(sigt5a, grepl("^Ex", sigt5a$cellType) | grepl("^In", sigt5a$cellType) )
...
dfsiggene_unique<- unique(dfsig$gene)
...
for (j in 1:length(sigt5$cellType)){
     ...
     for (i in 1:1000){
       set.seed(i)
       genelisttmp <- sample(dfsiggene_unique, genenum, replace = FALSE, prob = NULL)
       dfgenestmp <- filter(dfsig, gene %in% genelisttmp)
       median_rank <- median(dfgenestmp$corrSigRanked)
       null.dist.medians[i, 1] <- median_rank
     }
...
#Rank 4965 observations based on correlation
